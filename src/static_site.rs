...
impl StaticSite {
	...
	let mut pending_dirs_to_read: collections::VecDeque<path::PathBuf> = collections::VecDeque::new();
	...
	loop {
		...
		if let Some(dir_entries) = input_path.read_dir().ok() {
    ...
    fn get_txt_files_and_dirs(dir_path: &mut fs::ReadDir) -> (Vec<path::PathBuf>, Vec<path::PathBuf>) {
	...
	for dir in self.directory_paths.iter() {
	    let new_dir = output_folder_path.join(dir
				    .as_path()
	...
	for file in self.file_paths.iter() {
	    ...
	    index.push((page.title().to_owned().to_string(), relative_file_path.to_path_buf()));
	    let output_file_path = output_folder_path
		.join(relative_file_path);
	    
	    let output_folder_for_file = output_file_path
		.parent()
		.unwrap();
    ...
    fn create_index<P: AsRef<path::Path>>(output_folder: P, index: &[(String, path::PathBuf)]) -> io::Result<()> {
